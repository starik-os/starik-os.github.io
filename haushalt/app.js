...
const App = (() => {
  ...
  const init = async () => {
    ...
    try {
      ...
      // Seed-Import: Wenn DB leer und SEED_DATA vorhanden
      if (State.get().transactions.length === 0 && typeof SEED_DATA !== 'undefined' && SEED_DATA.length) {
        Debug.log('App', `Seed-Import: ${SEED_DATA.length} Buchungen...`);
        await Storage.putMany('transactions', SEED_DATA);
        await Transactions.loadAll();
        await Months.recalculateFrom('2025-06');
        Debug.log('App', 'Seed-Import abgeschlossen.');
      }

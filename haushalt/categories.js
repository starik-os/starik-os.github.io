...
const Categories = (() => {
  ...
  const DEFAULTS = [
    ...
    { id: 'miete',         fixed: true,         name: 'Miete',           icon: '🏠', color: '#9333ea', keywords: ['miete','grundstück','putzmann','erbengem','grundst'] },
    { id: 'strom',          fixed: true,         name: 'Strom/Gas',       icon: '💡', color: '#ea580c', keywords: ['strom','vattenfall','maingau','montana gas','gasabschlag','erdgas','heizgas','gas '] },
    { id: 'bvg',           name: 'BVG/Fahrkarte',   icon: '🚇', color: '#2563eb', keywords: ['bvg','fahrkarte','s-bahn','u-bahn','bolt '] },
    { id: 'versicherung',  fixed: true,  name: 'Versicherung',    icon: '🛡️', color: '#0f766e', keywords: ['versicherung','solidar','docura','barmer','hanse merkur','haftpflicht','sterbegeld'] },
    { id: 'unterhaltung',  fixed: true,  name: 'Unterhaltung',    icon: '🎬', color: '#7c3aed', keywords: ['netflix','disney','unterhaltung','spotify','chat gpt','chatgpt','google play','hp drucker','drucker','amazon prime'] },
    ...
    { id: 'internet',      fixed: true,      name: 'Internet/Handy',  icon: '📱', color: '#0284c7', keywords: ['vodafone','telekom','o2 ','internet','mobilfunk','simkarte','prepaid'] },
    ...
    { id: 'kontogebuehr',  fixed: true,  name: 'Kontogebühren',   icon: '🏦', color: '#6b7280', keywords: ['kontogebühr','kontoführung','sparkasse','n26 ','entgeld','bankkarte','kontoentgelt'] },
    { id: 'gez',           fixed: true,           name: 'GEZ/Rundfunk',    icon: '📺', color: '#374151', keywords: ['gez','rundfunk','beitragsservice'] },
    ...
    { id: 'schulden',      fixed: true,      name: 'Schulden/Raten',  icon: '📉', color: '#dc2626', keywords: ['schuldentilg','tilgung','rate ','kredit','darlehen','ba service'] },
  ...
  const isFixed = (id) => {
    ...
    return cat ? (cat.fixed === true) : false;
  };

  return { load, save, getAll, getById, detect, isFixed, reset, DEFAULTS };
})();
